...
def defineDiscriminator(harm_pred_model):
    ...
    output = layers.Dense(1, activation='sigmoid', name="harmonicity")(dropout2)
...
def defineGenerator(harm_pred_model):
    ...
    harm_pred_g = harm_pred_model(conv_out_g)
    ...
    deconv1_g_compare_g = keras.layers.Lambda(add)((deconv1_g, harm_diff_g))
    deconv2_compare_g = keras.layers.Conv2DTranspose(4, kernel_size=3, activation=keras.layers.LeakyReLU(0.2), padding="same")(deconv1_g_compare_g)
    conv_to_shape_compare_g = keras.layers.Conv1D(4, data_format="channels_last", kernel_size=3, activation=keras.layers.LeakyReLU(0.2), padding="valid")(deconv2_compare_g)
    conv_out_compare_g = layers.Conv2D(1, kernel_size=3, activation=keras.layers.LeakyReLU(0.2), padding="same")(conv_to_shape_compare_g)

    generator = keras.Model(inputs=[condition_input_g, harmonicity_input_g, noise_input_g], outputs=conv_out_compare_g, name="generator")
    generator.summary()
    return generator
...
discriminator = defineDiscriminator(harm_pred_model)
generator = defineGenerator(harm_pred_model)
...
for e in range(epochs):
    ...
    while True:
        try:
            ...
            d_loss_real, d_acc_real = discriminator.train_on_batch(x=[c, f, h], y=l)
            ...
            d_loss_fake, d_acc_fake = discriminator.train_on_batch(x=[c, fake_fixed, h], y=y_false)
