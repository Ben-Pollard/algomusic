...
//import transformers.RhythmTransformers.{shift, steps2Subdivisions}
//
...
//
//  val polyphonicPhrase = chordDegrees.map(d => ScalePhrase(melody.map(m => m+d), scale))
...
//
//  val bassPhrase = invert(transpose(ScalePhrase(melody, scale), -16))
//  val bassRhythm = shift(steps2Subdivisions(rhythm, 3),1,1)
//  val bassBar = Bar(bassPhrase :: Nil, bassRhythm)
//
////  val arrangement: ParallelBarSequences = List(BarSequence(List(bar)))
//  val arrangement: ParallelBarSequences = List(BarSequence(List(bar),1), BarSequence(List(bassBar),1))
