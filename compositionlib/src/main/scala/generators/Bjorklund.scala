...
object RhythmGenerators {
  ...
  private def euclidean(len: Int, notes:Int, list: List[List[Byte]]): List[List[Byte]] = {
    if (len == 1) {
      list
    } else {
  ...
  //4,3 = cumbia
  //16,5 = bossa nova
  def generateBjorklundSequence(len: Int, notes: Int, hitDuration: Duration, velocities: Seq[Velocity]): Rhythm = {
    generateBjorklundSequence(len, notes, List.fill(notes)(hitDuration), velocities)
  }

  def generateBjorklundSequence(steps: Int, hits: Int, velocities: Seq[Velocity]): Rhythm = {
    generateBjorklundSequence(steps, hits, List.fill(hits)(q), velocities)
  }

  def generateBjorklundSequence(steps: Int, hits: Int): Rhythm = {
    generateBjorklundSequence(steps, hits, List.fill(hits)(q), List.fill(hits)(100))
  }

  def generateBjorklundSequence(steps: Int, hits: Int, hitDuration: Duration): Rhythm = {
    generateBjorklundSequence(steps, hits, List.fill(hits)(hitDuration), List.fill(hits)(100))
  }

  def generateBjorklundSequence(len: Int, notes: Int, hitDurations: Seq[Duration], velocities: Seq[Velocity]): Rhythm = {
